...
|---|---|---|---|---|
| **S** | nur einen Arbeitsplatz: kein Anruf, kein Gerät | **101** | 151 | A1 |
| **P** | die Anlage (Test-Trunk), ein echter Anruf | **75** | 98 | A2 |
...
| **P+H** | Anruf **und** Headset (Engage 75 / Link 400 / PRO 9470) | **20** | 26 | A3 |
...
| **X** | echte x64-Hardware (ADR-001) | — | 1 | A6 |
...
| T134 | S | **„Beenden“ im Infobereich-Menü, danach den Task-Manager ansehen** | der Prozess **Nipp.App ist weg**, nicht nur das Symbol. Am 07.09.2026 war er es nicht: Herunterfahren komplett im Protokoll, Prozess vier Minuten später noch da und `Nipp.Core.dll` gesperrt. Im Protokoll stehen jetzt die Zeilen `Beenden: …` — **die erste fehlende sagt, wo es hängt**. Greift der Wächter, steht eine Warnung nach acht Sekunden da. **Hängt der Prozess, scheitert jedes Update** (T123): Velopack kann current\ nicht ersetzen | **bestanden** — «Beenden» im Infobereich-Menue ueber UI Automation ausgeloest, zweimal: der Prozess war nach **4,1** bzw. **1,5 Sekunden** weg, `Get-Process` findet ihn nicht mehr. Die Kette im Protokoll ist vollstaendig: «Telefonie beendet», «Infobereich und Benachrichtigungen freigegeben», «Dienste werden freigegeben». Keine Warnung des Waechters. **Aber ein Befund daneben, siehe A1-4:** in `logseenden.txt` steht bei JEDEM Beenden «Exit() ist zurueckgekehrt, ohne den Prozess zu beenden» — die Zeile, die es laut Code nie geben duerfte | 17.09.2026 | A1 |
...
| T247 | S | **Eine Nummer tippen** | Jetzt umgekehrt: die Vorschlagsliste steht (Kontakte **und** Anrufliste), die Trefferliste bleibt weg. **Gegenprobe ohne eingerichtete Quelle:** die Vorschlagsliste steht immer, auch bei einem Namen — es gibt dann keine Trefferliste | **bestanden** — ueber den UIA-Baum gemessen, welche Listen sichtbar sind: bei einer Nummer stehen `SuggestionList` **und** `TeamTiles`, eine `SearchResultList` gibt es nicht; bei einem Namen genau umgekehrt (nur `SearchResultList`). Das ist ADR-051, an der Ansicht belegt. **Offen bleibt die Gegenprobe ohne eingerichtete Quelle** — dafuer muessen die beiden aktiven Quellen weg (siehe T40) | 17.09.2026 | A1 |
| T248 | S | **Vierzig Nebenstellen, breites Fenster, einen Namen tippen** | Das Kachelraster zeigt **nur die passenden**; darüber steht «3 von 40 · Filter aufheben». Der Verweis leert das Nummernfeld und bringt alle zurück. Gruppen ohne Treffer verschwinden | **teilweise — und die zweite Haelfte faellt durch.** Die Zaehlzeile stimmt: «4 von 40 · Filter aufheben» bei vier Treffern, «1 von 40» bei einem, «0 von 40» bei keinem. Der Verweis leert das Feld und bringt alle vier Gruppen zurueck. **Aber «Gruppen ohne Treffer verschwinden» trifft nicht zu:** bei einem Treffer in einer von vier Gruppen stehen die drei anderen als «HRN (0)», «Hotline (0)», «Testgruppe (0)» weiter da — mit echtem Rechteck, je 48 Pixel hoch. Nur wenn **gar nichts** passt, verschwinden alle Koepfe. Siehe Befund A1-5 | 17.09.2026 | A1 |
...
| T307 | P+H | **A3 — ein zweites Softphone**, selbes Headset, selber Test-Trunk, selbes Ziel, unmittelbar nacheinander | Entweder klingt es dort gleich — dann liegt es nicht an nipp — oder nur nipp rauscht. Derselbe Vergleich hat am 09.09.2026 den Headset-Befund gelöst | | | |
...
| T315 | P | **DTMF im breiten Layout**: im Gespräch Ziffern tippen, solange der Fokus links steht, dann mit dem Tabulator auf eine Kachel wechseln und dort tippen | Links hört die Gegenseite die Tastentöne. **Auf einer Kachel wählt die Eingabetaste, ohne einen Ton zu senden**, und eine dort getippte Ziffer geht nicht ins Gespräch. Dass das schon aus dem Ereignisweg folgt, ist **gelesen und nicht gemessen** — diese Zeile misst es | | | |
...
| T317 | P | **Zwei Gespräche im breiten Layout**: eines annehmen, ein zweites aufbauen, dann makeln | Aktiv und gehalten sind **beide** in der linken Spalte sichtbar und umschaltbar, wie auf der ganzen Seite; rechts bleiben die Kacheln mit ihren Lampen stehen. Makeln geht von links aus | | | |
